...
import aiofiles.os
from changedetectionio.flask_app import event_wait
from changedetectionio.model import Watch
from changedetectionio.processors import ProcessorModuleProto
from changedetectionio.queuedWatchMetaData import PrioritizedItem
from changedetectionio.store import ChangeDetectionStore
from .processors.exceptions import ProcessorException
import changedetectionio.content_fetchers.exceptions as content_fetchers_exceptions
from changedetectionio.processors.text_json_diff.processor import FilterNotFoundInResponse
from changedetectionio import Sentinel, html_tools

import importlib
import os
import time
from asyncio import Event, Queue, PriorityQueue, QueueEmpty
...
class update_worker(object):
    current_uuid = None

    def __init__(self, q: PriorityQueue[PrioritizedItem], notification_q: Queue[dict[str, str] | Sentinel], exit_ev: Event, datastore: ChangeDetectionStore):
        self.q = q
        self.exit_ev = exit_ev
        self.notification_q = notification_q
        self.datastore = datastore

    async def queue_notification_for_watch(self, n_object, watch):
        ...
        if watch:
            watch_history = await watch.history
        ...
        await self.notification_q.put(n_object)
    ...
    async def send_content_changed_notification(self, watch_uuid):
        ...
        if n_object and n_object.get('notification_urls'):
            ...
            await self.datastore.update_watch(uuid=watch_uuid, update_obj={'notification_alert_count': count})

            await self.queue_notification_for_watch(n_object=n_object, watch=watch)

        return queued


    async def send_filter_failure_notification(self, watch_uuid):
    ...
    async def send_step_failure_notification(self, watch_uuid, step_n):
    ...
    async def cleanup_error_artifacts(self, uuid):
    ...
    async def run(self):
        ...
        # TODO(kruton): use Sentinel to signal that this task should break
        while not await event_wait(self.exit_ev, 1):
            update_handler = None

            try:
                queued_item_data = self.q.get_nowait()
            except QueueEmpty:
                continue
            ...
            if uuid in list(self.datastore.data['watching'].keys()) and self.datastore.data['watching'][uuid].get('url'):
                ...
                watch: Watch.model = self.datastore.data['watching'].get(uuid)
                ...
                try:
                    ...
                    try:
                        processor_module = cast(ProcessorModuleProto, importlib.import_module(processor_module_name))
                    ...
                    changed_detected, update_obj, contents = await update_handler.run_changedetection(
                        watch=watch,
                ...
                # A generic other-exception thrown by processors
                    ...
                    await self.datastore.update_watch(uuid=uuid, update_obj={'last_error': e.message})
                    process_changedetection_results = False
                    ...
                    await self.datastore.update_watch(uuid=uuid, update_obj={
                    ...
                    await self.datastore.update_watch(uuid=uuid, update_obj={'last_error': err_text})
                    process_changedetection_results = False
                    ...
                    if watch.get('filter_failure_notification_send', False):
                        ...
                        if threshold > 0 and c >= threshold:
                            if not watch.get('notification_muted'):
                                await self.send_filter_failure_notification(uuid)
                            c = 0

                        await self.datastore.update_watch(uuid=uuid, update_obj={'consecutive_filter_failures': c})
                    ...
                    await self.datastore.update_watch(uuid=uuid,
                    ...
                    if watch.get('filter_failure_notification_send', False):
                        ...
                        if threshold > 0 and c >= threshold:
                            if not watch.get('notification_muted'):
                                await self.send_step_failure_notification(watch_uuid=uuid, step_n=e.step_n)
                            c = 0
                    ...
                    logger.error(str(e))
                    await self.datastore.update_watch(uuid=uuid, update_obj={'last_error': "Exception: " + str(e)})
                    # Other serious error
                    process_changedetection_results = False
                    ...
                    await self.cleanup_error_artifacts(uuid)
                ...
                if process_changedetection_results:
                    ...
                    try:
                        await self.datastore.update_watch(uuid=uuid, update_obj=update_obj)
                        ...
                        if changed_detected or not watch.history_n:
                            ...
                            if await watch.newest_history_key and int(timestamp) == int(await watch.newest_history_key):
                                logger.warning(
                                    f"Timestamp {timestamp} already exists, waiting 1 seconds so we have a unique key in history.txt")
                                timestamp = str(int(timestamp) + 1)
                                await asyncio.sleep(1)

                            await watch.save_history_text(contents=contents,
                                                    timestamp=timestamp,
                                                    snapshot_id=update_obj.get('previous_md5', 'none'))

                            if update_handler.fetcher.content:
                                await watch.save_last_fetched_html(contents=update_handler.fetcher.content, timestamp=timestamp)

                            # Notifications should only trigger on the second time (first time, we gather the initial snapshot)
                            if watch.history_n >= 2:
                                logger.info(f"Change detected in UUID {uuid} - {watch['url']}")
                                if not watch.get('notification_muted'):
                                    await self.send_content_changed_notification(watch_uuid=uuid)
                        ...
                        await self.datastore.update_watch(uuid=uuid, update_obj={'last_error': str(e)})
                ...
                await self.datastore.update_watch(uuid=uuid, update_obj={'fetch_time': round(time.time() - now, 3),
