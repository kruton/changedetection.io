from quart import Blueprint, request, render_template, flash, url_for, redirect
...
def construct_blueprint(datastore: ChangeDetectionStore):
    ...
    @tags_blueprint.route("/list", methods=['GET'], endpoint='tags_overview_page')
    @login_optionally_required
    async def tags_overview_page():
        from .form import SingleTag
        add_form = SingleTag(await request.form)
        ...
        output = await render_template("groups-overview.html",
    ...
    @tags_blueprint.route("/add", methods=['POST'], endpoint='form_tag_add')
    @login_optionally_required
    async def form_tag_add():
        from .form import SingleTag

        form = await request.form
        add_form = SingleTag(form)
        ...
        if datastore.tag_exists_by_name(title):
            await flash(f'The tag "{title}" already exists', "error")
            return redirect(url_for('tags.tags_overview_page'))

        datastore.add_tag(title)
        await flash("Tag added")

        return redirect(url_for('tags.tags_overview_page'))

    @tags_blueprint.route("/mute/<string:uuid>", methods=['GET'], endpoint='mute')
    @login_optionally_required
    def mute(uuid):
    ...
    @tags_blueprint.route("/delete/<string:uuid>", methods=['GET'], endpoint='delete')
    @login_optionally_required
    async def delete(uuid):
        ...
        await flash(f"Tag deleted and removed from {removed} watches")
        return redirect(url_for('tags.tags_overview_page'))

    @tags_blueprint.route("/unlink/<string:uuid>", methods=['GET'], endpoint='unlink')
    @login_optionally_required
    async def unlink(uuid):
        ...
        await flash(f"Tag unlinked removed from {unlinked} watches")
        return redirect(url_for('tags.tags_overview_page'))

    @tags_blueprint.route("/delete_all", methods=['GET'], endpoint='delete_all')
    @login_optionally_required
    async def delete_all():
        ...
        await flash(f"All tags deleted")
        return redirect(url_for('tags.tags_overview_page'))

    @tags_blueprint.route("/edit/<string:uuid>", methods=['GET'], endpoint='form_tag_edit')
    @login_optionally_required
    async def form_tag_edit(uuid):
        ...
        form = group_restock_settings_form(formdata=await request.form if request.method == 'POST' else None,
                                       data=default,
        ...
        output = await render_template("edit-tag.html",
    ...
    @login_optionally_required
    async def form_tag_edit_submit(uuid):
        from changedetectionio.blueprint.tags.form import group_restock_settings_form
        if uuid == 'first':
            uuid = list(datastore.data['settings']['application']['tags'].keys()).pop()
