...
class Flash(Protocol):
    def __call__(self, message: str, category: str = "message") -> Coroutine[Any, Any, None]:
        ...
...
class Importer():
    ...
    def __init__(self):
        ...
        self.good = 0
        self.remaining_data = []
        self.import_profile: dict[int, str] = {}
...
class import_url_list(Importer):
    ...
    async def run(self,
        ...
        if (len(urls) > 5000):
            await flash("Importing 5,000 of the first URLs from your list, the rest can be imported again.")
        ...
        for url in urls:
            ...
            if len(url) and 'http' in url.lower() and good < 5000:
                ...
                new_uuid = await datastore.add_watch(url=url.strip(), tag=tags, write_to_disk_now=False, extras=extras)
        ...
        await flash("{} Imported from list in {:.2f}s, {} Skipped.".format(good, time.time() - now, len(self.remaining_data)))
...
class import_distill_io_json(Importer):
    async def run(self,
        ...
        try:
            ...
            await flash("Unable to read JSON file, was it broken?", 'error')
            return

        if not data.get('data'):
            await flash("JSON structure looks invalid, was it broken?", 'error')
            return
        ...
        for d in data.get('data'):
            ...
            if len(d['uri']) and good < 5000:
                ...
                new_uuid = await datastore.add_watch(url=d['uri'].strip(),
                                               tag=",".join(d.get('tags', [])),
                                               extras=extras,
        ...
        await flash("{} Imported from Distill.io in {:.2f}s, {} Skipped.".format(len(self.new_uuids), time.time() - now, len(self.remaining_data)))
...
class import_xlsx_wachete(Importer):
    ...
    async def run(self,
        ...
        try:
            ...
            # @todo correct except
            await flash("Unable to read export XLSX file, something wrong with the file?", 'error')
            return

        if not wb.active:
            await flash("Unable to read export XLSX file, workbook is not active", "error")
            return
            ...
            try:
                ...
                if data.get('url'):
                    try:
                        ...
                        await flash(f"Error processing row number {row_id}, URL value was incorrect, row was skipped.", 'error')
                        # Don't bother processing anything else on this row
                        continue

                    new_uuid = await datastore.add_watch(url=data['url'].strip(),
                                                   extras=extras,
                                                   tag=data.get('folder'),
                                                   write_to_disk_now=False)
                    if new_uuid:
                ...
                logger.error(e)
                await flash(f"Error processing row number {row_id}, check all cell data types are correct, row was skipped.", 'error')
            else:
                row_id += 1
...
class import_xlsx_custom(Importer):
    ...
    async def run(self,
        ...
        if not wb.active:
            await flash("Unable to read export XLSX file, worksheet would not load.", 'error')
            return
        ...
        try:
            for row in wb.active.iter_rows():
                ...
                for cell in row:
                    ...
                    if cell_map == 'url':
                        ...
                        # @todo validate?
                        include_filters = cell.value
                        if type(include_filters) == str:
                            include_filters = include_filters.strip()
                ...
                # At minimum a URL is required.
                if url:
                    new_uuid = await datastore.add_watch(url=url,
                                                   extras=extras,
                                                   tag=tags,
                                                   write_to_disk_now=False)
                    if new_uuid:
            ...
            logger.error(e)
            await flash(f"Error processing row number {row_i}, check all cell data types are correct, row was skipped.", 'error')
        else:
            row_i += 1
